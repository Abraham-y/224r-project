...
Pipeline (per the 5-step spec):
  ...
  Step 1 -- Per-problem probe AUROC under each checkpoint.
    ...
    the layer-L hidden state at the **`<answer>` opening token of the LAST
    `<answer>` block** -- the SAME position as the §2.4 position-appropriate
    probe in writeup.md (held-out AUROC 0.920). The position is configurable
    via --answer_occurrence / --answer_token_pos but the defaults match the
    writeup. Run the linear probe on it to get a scalar score, and read the
    verifier's binary correctness label (stored score == 1.0). AUROC over
    the K (probe_score, label) pairs is the per-problem probe quality.
...
def main() -> None:
    ...
    parser.add_argument("--answer_token_pos", choices=("after", "open"), default="open",
                        help="'open' (default) places the probe at the <answer> opening "
                             ...
                             "probe in writeup.md (held-out AUROC 0.920). 'after' uses the "
                             "token immediately following the tag.")
