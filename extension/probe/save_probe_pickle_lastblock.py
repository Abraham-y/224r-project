"""Train + pickle the C_outcome pre_answer L16 probe with ROLLOUT-FINAL labels
(matches what the vanilla RLOO verifier rewards: the LAST <answer> block).

This is the apples-to-apples probe for the probe-RL-vs-vanilla-RLOO comparison:
- vanilla RLOO reward = verifier(LAST <answer>) ∈ {0, 0.1, 1.0}
- probe-RL (this probe) reward = probe predicting verifier(LAST <answer>)

Held-out balanced AUROC will be ~0.90 (vs 0.98 for first-block-corrected).
The noisier proxy is the more interesting Goodhart probe -- more room for
the policy to push probe up without genuinely improving accuracy.

Outputs:
  extension/cache/steering/probe_pipeline_C_outcome_l16_pre_answer_lastblock.pkl
  extension/cache/steering/probe_pipeline_lastblock_meta.json
"""
...
PKL = f"{OUT_DIR}/probe_pipeline_C_outcome_l16_pre_answer_lastblock.pkl"
META_OUT = f"{OUT_DIR}/probe_pipeline_lastblock_meta.json"
...
def main():
    ...
    # Label by rollout-final verifier score (== 1.0 if last <answer> block is
    # correct under the standard Countdown verifier). Same labels vanilla
    # RLOO uses as its reward target.
    labs = {}
    ...
    print(f"held-out balanced AUROC (rollout-final labels): {auroc:.3f}")
    ...
    with open(META_OUT, "w") as f:
        json.dump({
            ...
            "label_rule": "rollout-final verifier correctness (matches vanilla RLOO reward)",
