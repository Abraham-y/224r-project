...
_os.environ["PYTHONPATH"] = _os.pathsep.join(
    [x for x in (str(_ROOT), _os.environ.get("PYTHONPATH", "")) if x])
...
def parse_args():
    ...
    parser.add_argument('--output_dir', type=str, default='/vol/evaluation/eval_results')
...
if __name__ == "__main__":
    ...
    # FORK: accept a local .jsonl as well as an HF dataset name. The paper's
    # eval population is extension/data/countdown_eval_500.jsonl (500 prompts,
    # filtered to the clean 406 downstream); `load_dataset(name, split='test')`
    # cannot address a local file, which is what made the original fail here.
    if str(args.eval_dataset).endswith((".jsonl", ".json")):
        loaded_dataset = load_dataset("json", data_files=str(args.eval_dataset), split="train")
    else:
