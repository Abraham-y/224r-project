...
REMOTE_PROJECT_ROOT = "/root/default_proj"
REMOTE_VOLUME_ROOT = "/vol"
REMOTE_REQUIREMENTS_PATH = f"{REMOTE_PROJECT_ROOT}/modal_requirements.txt"
...
def _run_training(script_path: str, trainer_args: list[str]) -> str:
    ...
    (vol_root / "checkpoints").mkdir(parents=True, exist_ok=True)

    env = os.environ.copy()
    hf_home = vol_root / "cache" / "huggingface"
...
def _run_eval(eval_args: list[str]) -> str:
    ...
    vol_root.mkdir(parents=True, exist_ok=True)
    (vol_root / "cache" / "huggingface" / "datasets").mkdir(parents=True, exist_ok=True)
    (vol_root / "evaluation" / "eval_results").mkdir(parents=True, exist_ok=True)
