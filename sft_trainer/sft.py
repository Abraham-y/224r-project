    ...
    for i in range(num_epochs):
        for batch in train_dataloader:
            ...
            with torch.no_grad():
                ...
                correct = (predicted_tokens == targets) & mask.bool()
                train_accuracy = correct.sum() / mask.sum().clamp(min=1)
            ...
            if microbatch_step % gradient_accumulation_steps == 0:
                ...
                if optimizer_step % 10 == 0:
                    wandb.log({"train/loss": loss.item(), "train/accuracy": train_accuracy.item(), "train/lr": scheduler.get_last_lr()[0]}, step=optimizer_step)            
                if optimizer_step % eval_every == 0:
                    model.eval()
                    total_eval_loss = 0
                    total_eval_tokens = 0
                    ...
                    with torch.no_grad():
                        for eval_batch in test_dataloader:
                            ...
                            eval_is_response_token = eval_batch["is_response_token"].to(device)
                            ...
                            eval_pred_tokens = eval_predictions.argmax(dim=-1)
                            eval_correct = (eval_pred_tokens == eval_targets) & eval_mask.bool()
                            total_eval_correct += eval_correct.sum().item()
                    
                    avg_loss = total_eval_loss / max(total_eval_tokens, 1)
                    avg_accuracy = total_eval_correct / max(total_eval_tokens, 1)
                    wandb.log({"eval/loss": avg_loss, "eval/accuracy": avg_accuracy}, step=optimizer_step)
                    model.train()
    model.eval()
    total_eval_loss = 0
    total_eval_tokens = 0
